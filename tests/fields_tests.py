...
def run():
    ...
    check(f"вчера в 09:00 -> {today - timedelta(days=1)}",      parse_date("вчера в 09:00"),        today - timedelta(days=1))
    check(f"2 дня назад -> {today - timedelta(days=2)}",        parse_date("2 дня назад"),          today - timedelta(days=2))
    check("15 марта 2024 -> 2024-03-15",                        parse_date("15 марта 2024"),        date(2024, 3, 15))
    check(f"15 марта -> {date(today.year, 3, 15)}",             parse_date("15 марта"),             date(today.year, 3, 15))
