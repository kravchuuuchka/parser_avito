...
def run():
    ...
    for col_idx, (header, _, _w) in enumerate(COLUMNS, 1):
        check(f"заголовок колонки {col_idx}: «{header}»",
    ...
    for row in rows:
        ...
        check(f"{ad_id}: цена {expected_price}",
              ws.cell(r, 3).value, expected_price)

        expected_status = "Активно" if row["status"] == 1 else "Закрыто"
        check(f"{ad_id}: статус - {expected_status}",
              ws.cell(r, 8).value, expected_status)

        published_on = _str_to_date(row["published_on"])
        if published_on:
            expected_date = published_on.strftime("%d.%m.%Y")
            check(f"{ad_id}: дата публикации - {expected_date}",
                  ws.cell(r, 6).value, expected_date)

        cached_at_val  = ws.cell(r, 12).value
        updated_at_val = ws.cell(r, 13).value
        check(f"{ad_id}: cached_at заполнен",
              cached_at_val is not None, True)
        check(f"{ad_id}: updated_at заполнен",
              updated_at_val is not None, True)

        cached_dt = _str_to_dt(row["cached_at"])
        if cached_dt:
            expected_cached = cached_dt.strftime("%d.%m.%Y %H:%M:%S")
            check(f"{ad_id}: cached_at отформатирован верно",
                  cached_at_val, expected_cached)
